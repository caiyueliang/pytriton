...
pipe = StableDiffusionPipeline.from_pretrained("/mnt/publish-data/pretrain_models/text2image/stable-diffusion-2-1", torch_dtype=torch.float16)
pipe = pipe.to(DEVICE)
...
def _infer_fn(
    ...
    logger.debug(f"Prompts: len: {len(prompts)}; {prompts}")
        ...
        triton.bind(
            ...
            config=ModelConfig(
                ...
                batcher=DynamicBatcher(
                    max_queue_delay_microseconds=100*1000,
                ),
            ),
            strict=True,
        )
